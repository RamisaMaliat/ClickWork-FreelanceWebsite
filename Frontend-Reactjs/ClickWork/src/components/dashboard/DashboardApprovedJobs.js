    ...
    <dl className="dictionary">
      {posts.map(post => (
        <div className="term" style={{"marginBottom":"70px"}}>
          ...
          <dd><b>{"Client Email" + " " + ":" + " " + " "}</b>{post.ClientEmail}</dd><br/>
          <dd><b style={{"color":"green"}}>{"Rating" + " " + ":" + " " + " "}</b><b><random style={{"fontSize":"18px"}}>{post.Rating}{".00 / 5.00"}</random></b></dd>
          {post.ClientComment?<dd><b style={{"color":"green"}}>{"Client's Comment" + " " + ":" + " " + " "}</b><br/><i>{post.ClientComment}</i></dd>:""}
...
const ViewJobs = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      const res = await axios.get(process.env.REACT_APP_API + 'freelancer/viewapprovedjobs',
        {
          headers: {
...
class DashboardApprovedJobs extends Component {
  onLogout = (e) => {
    e.preventDefault();
    this.props.logout();
  };
      ...
      <app>
        ...
        <div class="container" style={{ "height": "100px", "paddingTop": "0px" }} >
          <ul class="nav nav-tabs" >
            ...
            <li role="presentation" class="active" style={{ "paddingRight": "40px" }}><a>Approved Jobs</a></li>
            <li role="presentation" style={{ "paddingRight": "40px" }}><a href="myinvitations">My Invitations</a></li>
            <li role="presentation" style={{ "paddingRight": "40px" }}><a href="myproposals">My Proposals</a></li>
          </ul>
        </div>
...
DashboardApprovedJobs.propTypes = {
...
export default connect(mapStateToProps, {
  logout
})(withRouter(DashboardApprovedJobs));