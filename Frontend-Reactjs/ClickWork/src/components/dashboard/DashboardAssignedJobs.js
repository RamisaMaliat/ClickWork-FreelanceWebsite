    ...
    <dl className="dictionary">
      {posts.map(post => (
        <div className="term" style={{"marginBottom":"70px"}}>
          ...
          <h4 style={{"paddingTop":"20px"}}><b>Assigned By : </b></h4>
          <dd><b>{"Client Name" + " " + ":" + " " + " "}</b>{post.ClientName}</dd>
          <dd><b>{"Client Email" + " " + ":" + " " + " "}</b>{post.ClientEmail}</dd>
          <b><div style={{"marginTop":"30px","fontSize":"16px"}}> Did you complete your work? If yes,
          <a href={"/jobs/completed/"+post.JobID} > Send request to client to review your work </a>.
          </div></b>
        </div>
...
const ViewJobs = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      const res = await axios.get(process.env.REACT_APP_API + 'freelancer/viewassignedjobs',
        {
          headers: {
...
class DashboardAssignedJobs extends Component {
  onLogout = (e) => {
    e.preventDefault();
    this.props.logout();
  };
      ...
      <app>
        ...
        <div class="container" style={{ "height": "100px", "paddingTop": "0px" }} >
          <ul class="nav nav-tabs" >
            <li role="presentation" class="active" style={{ "paddingRight": "40px" }}><a>Assigned Jobs</a></li>
...
DashboardAssignedJobs.propTypes = {
...
export default connect(mapStateToProps, {
  logout
})(withRouter(DashboardAssignedJobs));